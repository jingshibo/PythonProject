...
class ImageDataset(Dataset):
    def __init__(self, root, transform=None, mode='train'):
        ...
        self.folder_A = "D:\Data\Week 3\horse2zebra\\trainB"
        self.folder_B = "D:\Data\Week 3\horse2zebra\\trainB"
        self.files_A = os.listdir(self.folder_A)
        self.files_B = os.listdir(self.folder_B)
    ...
    def __getitem__(self, index):
        image_A_path = os.path.join(self.folder_A, self.files_A[index])
        image_B_path = os.path.join(self.folder_B, self.files_B[index])
        item_A = self.transform(Image.open(image_A_path))
        item_B = self.transform(Image.open(image_B_path))
        if item_A.shape[0] != 3:
...
transform = transforms.Compose([
    ...
    transforms.ToTensor(), # convert pixel value into the range of [0,1]
])
...
for img, _ in dataset:
    min_value = min(min_value, torch.min(img))

print('Minimum pixel value in the dataset:', min_value)
