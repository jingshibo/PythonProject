...
class Generator_UNet(nn.Module):
    def __init__(self, input_vector_dim, img_height, img_width, output_chan, hidden_channels=32):
        ...
        # decoder
        self.expand4 = ExpandingBlock(hidden_channels * 16, stride=1, SN=True)  # Adjusted for skip connection
        self.expand3 = ExpandingBlock(hidden_channels * 8, stride=1, SN=True)  # Adjusted for skip connection
        self.expand2 = ExpandingBlock(hidden_channels * 4, stride=1, SN=True)  # Adjusted for skip connection
        self.expand1 = ExpandingBlock(hidden_channels * 2, stride=1, SN=True)  # Adjusted for skip connection
        self.downfeature = FeatureMapBlock(int(hidden_channels * 1), output_chan, stride=1, SN=True)
    ...
    def forward(self, noise_and_class):
        ...
        y4 = self.expand4(x4)  # Skip connection from contract1 to expand2
        y3 = self.expand3(y4)  # Skip connection from contract1 to expand2
        y2 = self.expand2(y3)  # Skip connection from upfeature to expand3
        y1 = self.expand1(y2)  # Skip connection from upfeature to expand3
