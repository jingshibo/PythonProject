...
# train classifier
new_evaluation = cGAN_Evaluation.cGAN_Evaluation(gen_results, window_parameters)
train_set, shuffled_train_set = new_evaluation.classifierTrainSet(filtered_new_fake_data, training_percent=0.8)
...
# plot values of certain transition type
Plot_Emg_Data.plotMultipleEventMeanValues(fake_new_2, real_new, modes, title='emg_2_on_2', ylim=(0, 0.5))
...
'''
## build training data
old_emg_for_replacement = {modes[2]: filtered_old_fake_data[modes[2]] for transition_type, modes in modes_generation.items()}
mix_old_new_data = Process_Fake_Data.replaceUsingFakeEmg(old_emg_for_replacement, filtered_new_fake_data)
...
# test classifier
shuffled_test_set = new_evaluation.classifierTestSet(modes_generation, filtered_new_fake_data, train_set, test_ratio=0.5)
