...
class ContractingBlock(nn.Module):
    def __init__(self, input_channels, num_classes, use_bn='batch_norm', stride=1, kernel_size=3, padding=1, activation='relu', SN=False):
        ...
        if use_bn == 'batch_norm' or use_bn == 'instance_norm':
            self.norm = ConditionalNorm2d(input_channels * 2, num_classes, norm=use_bn)
            self.use_bn = True
        else:
            self.use_bn = False
...
class Generator_UNet(nn.Module):
    def __init__(self, input_vector_dim, img_height, img_width, output_chan, num_classes, hidden_channels=32):
        ...
        self.contract1 = ContractingBlock(hidden_channels, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)
        self.contract2 = ContractingBlock(hidden_channels * 2, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)
        self.contract3 = ContractingBlock(hidden_channels * 4, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)
        self.contract4 = ContractingBlock(hidden_channels * 8, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)

        # decoder
        self.expand4 = ExpandingBlock(hidden_channels * 16 + hidden_channels * 8, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)
        self.expand3 = ExpandingBlock(hidden_channels * 12 + hidden_channels * 4, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)
        self.expand2 = ExpandingBlock(hidden_channels * 8 + hidden_channels * 2, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)
        self.expand1 = ExpandingBlock(hidden_channels * 5 + hidden_channels, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, SN=True)
...
class Discriminator_Same(nn.Module):
    def __init__(self, input_channels, num_classes, hidden_channels=32):
        ...
        self.contract2 = ContractingBlock(hidden_channels * 2, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, activation='lrelu', SN=True)
        self.contract3 = ContractingBlock(hidden_channels * 4, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, activation='lrelu', SN=True)
        self.contract4 = ContractingBlock(hidden_channels * 8, num_classes, use_bn='batch_norm', kernel_size=3, padding=1, activation='lrelu', SN=True)
