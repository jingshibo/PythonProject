...
sliced_old_real_data, window_parameters = Post_Process_Data.sliceEmgData(processed_old_real_data, start=start_index, end=end_index,
    toeoff=start_before_toeoff_ms, sliding_results=False)
sliced_new_real_data, _ = Post_Process_Data.sliceEmgData(processed_new_real_data, start=start_index, end=end_index,
    toeoff=start_before_toeoff_ms, sliding_results=False)
...
processed_old_real_data = Process_Fake_Data.reorderSmoothDataSet(old_real_emg_grids['grid_1'], filtering=False, modes=None)
sliced_old_fake_data, window_parameters = Post_Process_Data.sliceEmgData(processed_old_fake_data, start=start_index, end=end_index,
    toeoff=start_before_toeoff_ms, sliding_results=False)
sliced_old_real_data, _ = Post_Process_Data.sliceEmgData(processed_old_real_data, start=start_index, end=end_index,
    toeoff=start_before_toeoff_ms, sliding_results=False)
...
selected_old_fake_data = Dtw_Similarity.extractFakeData(sliced_old_fake_data, sliced_old_real_data, modes_generation, envelope_frequency=50,
    num_sample=200, num_reference=1, method='select', random_reference=False, split_grids=True)
filtered_old_fake_data = Post_Process_Data.spatialFilterModelInput(selected_old_fake_data['fake_data_based_on_grid_1'],
    kernel=classifier_filter_kernel)
filtered_old_real_data = Post_Process_Data.spatialFilterModelInput(sliced_old_real_data, kernel=classifier_filter_kernel)
...
## plotting fake and real emg data for comparison
# calculate average values
# reference_old_data={transition_type:[sliced_old_real_data[transition_type][index]forindexin
...
processed_new_real_data = Process_Fake_Data.reorderSmoothDataSet(new_real_emg_grids['grid_1'], filtering=False, modes=None)
sliced_new_fake_data, window_parameters = Post_Process_Data.sliceEmgData(processed_new_fake_data, start=start_index, end=end_index,
    toeoff=start_before_toeoff_ms, sliding_results=False)
sliced_new_real_data, _ = Post_Process_Data.sliceEmgData(processed_new_real_data, start=start_index, end=end_index,
    toeoff=start_before_toeoff_ms, sliding_results=False)
...
selected_new_fake_data = Dtw_Similarity.extractFakeData(sliced_new_fake_data, sliced_new_real_data, modes_generation, envelope_frequency=50,
    num_sample=200, num_reference=1, method='select', random_reference=False, split_grids=True)
filtered_new_fake_data = Post_Process_Data.spatialFilterModelInput(selected_new_fake_data['fake_data_based_on_grid_1'],
    kernel=classifier_filter_kernel)
filtered_new_real_data = Post_Process_Data.spatialFilterModelInput(sliced_new_real_data, kernel=classifier_filter_kernel)
...
train_set, shuffled_train_set = new_evaluation.classifierTlTrainSet(filtered_new_fake_data, reference_new_real_data,
    dataset='cross_validation_set', minimum_train_number=10)
...
sliced_mix_data, window_parameters = Post_Process_Data.sliceEmgData(processed_mix_data, start=start_index, end=end_index,
    toeoff=start_before_toeoff_ms, sliding_results=False)
# median filtering
