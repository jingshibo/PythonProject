...
def getSubjectResults(subject, version, result_set):
    ...
    accuracy = {'accuracy_best': accuracy_best, 'accuracy_new': accuracy_new, 'accuracy_compare': accuracy_compare,
        'accuracy_noise': accuracy_noise, 'accuracy_worst': accuracy_worst}
    cm_call = {'cm_recall_best': cm_recall_best, 'cm_recall_new': cm_recall_new, 'cm_recall_compare': cm_recall_compare,
        'cm_recall_noise': cm_recall_noise, 'cm_recall_worst': cm_recall_worst}
...
def calcuStatValues(combined_results):
    ...
    # Calculate the mean and std values for each column and add them to the DataFrame
    for model_name, model_value in mean_std_value["accuracy"].items():
        # Calculate mean and std for each column and add them to the DataFrame
        model_value.loc["mean"] = model_value.mean()
        model_value.loc["std"] = model_value.std()
    ...
    for model_name, model_value in combined_results["cm_call"].items():
        ...
        for delay_key, delay_values in model_value.items():
            ...
            mean_std_value["cm_call"][model_name][delay_key] = np.mean(arrays, axis=0)
