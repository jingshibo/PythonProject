...
from Conditional_GAN.Data_Procesing import Process_Fake_Data, Read_Raw_Data
import datetime


'''train generative model'''
##  define windows
window_parameters = Read_Raw_Data.returnWindowParameters()
...
old_emg_data = Read_Raw_Data.readFilterEmgData(data_source, window_parameters, lower_limit=20, higher_limit=400)
...
sessions = [up_down_session, down_up_session]
data_source = {'subject': subject, 'version': version, 'modes': modes_generation, 'sessions': sessions}
new_emg_data = Read_Raw_Data.readFilterEmgData(data_source, window_parameters, lower_limit=20, higher_limit=400)


## normalize and extract emg data for gan model training
range_limit = 1500
old_emg_normalized, new_emg_normalized, old_emg_reshaped, new_emg_reshaped = Read_Raw_Data.normalizeReshapeEmgData(old_emg_data, new_emg_data, range_limit)
...
time_interval = 50
extracted_emg, train_gan_data = Read_Raw_Data.extractSeparateEmgData(modes_generation, old_emg_reshaped, new_emg_reshaped, time_interval, length, output_list=True)
...
acc, cm = Model_Storage.loadClassifyAccuracy(subject, version, result_set, model_type, project='cGAN_Model')
