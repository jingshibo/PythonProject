# This is for only one insole to connect
...
class Insole:
    ...
    def load_lib(self, address):
        ...
        self.clib.InitUser(0, 0, 0, 0, 1, 1, 0) # one insole
        if address == self.left_insole_address:
            self.clib.SetSensorSpecNew(0, 0, 0, 0, 0, 0, 0, 0, 3, 22, 1)
        elif address == self.right_insole_address:
    ...
    async def connectInsole(self, address):
        try:
            if address == self.left_insole_address: # connect to usb ble adaptor
                self.client = BleakClient(address, adapter="hci0")  # use "busctl tree org.bluez" to obtain adapter name
                print("connect to", address)
                await self.client.connect()
                await self.transmitData(self.client)
            elif address == self.right_insole_address: # connect to internal ble adaptor
                self.client = BleakClient(address, adapter="hci1")
                print("connect to", address)
                await self.client.connect()
                await self.transmitData(self.client)
        except Exception as e:
    ...
    def retrievInsoleData(self, side: int):
