...
## design model
class Raw_Cnn_4d(nn.Module):
    def __init__(self, input_size, class_number):
        super(Raw_Cnn_4d, self).__init__()

        # define layer parameter
        self.conv1_parameter = [32, 5]
        ...
        self.convolutional_layer = nn.Sequential(
            nn.Conv3d(in_channels=input_size, out_channels=self.conv1_parameter[0], kernel_size=self.conv1_parameter[1], dilation=1, stride=1),
            nn.BatchNorm3d(self.conv1_parameter[0]),
            nn.ReLU(),
            nn.AvgPool3d(kernel_size=2, stride=1, padding=0),

            nn.Conv3d(in_channels=self.conv1_parameter[0], out_channels=self.conv2_parameter[0], kernel_size=self.conv2_parameter[1], dilation=1, stride=1),
            nn.BatchNorm3d(self.conv2_parameter[0]),
            nn.ReLU(),
            nn.AvgPool3d(kernel_size=2, stride=1, padding=0),

            nn.Conv3d(in_channels=self.conv2_parameter[0], out_channels=self.conv3_parameter[0], kernel_size=self.conv3_parameter[1], dilation=1, stride=2),
            nn.BatchNorm3d(self.conv3_parameter[0]),
            nn.ReLU(),
            nn.AvgPool3d(kernel_size=2, stride=1, padding=0)
        )
...
# summary(model, input_size=(1024, 1, 512, 13, 10))
...
class ModelTraining():
    ...
    def trainModel(self, shuffled_groups, decay_epochs, select_channels='emg_all'):
        ...
        for group_number, group_value in {'group_1': shuffled_groups['group_1']}.items():
            ...
            # extract the dataset
            input_size = group_value['train_feature_x'].shape[3]
            ...
            # training parameters
            self.model = Raw_Cnn_4d(input_size, class_number).to(self.device)  # move the model to GPU
    ...
    def selectSamples(self, group_value, select_channels, bipolar_position=(0, 0)):
        ...
        if select_channels == 'emg_all':
            ...
            train_set_x = group_value['train_feature_x'][:, :, 0:5, :, :]
            test_set_x = group_value['train_feature_x'][:, :, 0:5, :, :]
        elif select_channels == 'emg_2':
            train_set_x = group_value['train_feature_x'][:, :, 5:10, :, :]
            test_set_x = group_value['train_feature_x'][:, :, 5:10, :, :]
...
class EmgDataSet(Dataset):
    def __init__(self, shuffled_data, mode):
        ...
        self.x_data = torch.from_numpy(np.transpose(shuffled_data[f'{mode}_feature_x'], (4, 3, 0, 1, 2)))  # size [n_samples, n_channel, length, width]
