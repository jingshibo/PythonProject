...
class Raw_Cnn_Rnn(nn.Module):
    def __init__(self, input_size, class_number):
       ...
       # define dense layer
        ...
        self.rnn_layers = nn.GRU(input_size=self.linear1_parameter, hidden_size=self.rnn1_parameter, num_layers=2, batch_first=True)
...
# summary(model, input_size=(256, 6, 1, 350, 130))
...
class EmgDataSet(Dataset):
    ...
    def __getitem__(self, index):   # support indexing such that dataset[i] can be used to get i-th sample
        ...
        y = torch.from_numpy(np.array([self.y_data[index]]))  # size [n_samples]
        return x, y
