...
def seperateGait(split_data, window_size=512):
    ...
    SSLW = pd.concat([split_data["SSLW"] - window_size, split_data["SSLW"] + window_size], axis=1)
    SASA = pd.concat([split_data["SASA"] - window_size, split_data["SASA"] + window_size], axis=1)
    SDSD = pd.concat([split_data["SDSD"] - window_size, split_data["SDSD"] + window_size], axis=1)
    ...
    if 'LWSA' and 'SSSD' and 'SASS' and 'SDLW' in split_data.columns:  # if it is up_dowm experiment
        LWSA = pd.concat([split_data["LWSA"] - window_size, split_data["LWSA"] + window_size], axis=1)
