...
def classifyGtuLastOneModel(shuffled_groups, time_steps):
    results = []
    for group_number, group_value in shuffled_groups.items():
        ...
        model.add(tf.keras.layers.GRU(1000, return_sequences=True, dropout=0.5, input_shape=(time_steps, train_set_x.shape[2])))
...
def classifyMultipleGruModel(shuffled_groups, time_steps):
    group_results = []
    for group_number, group_value in shuffled_groups.items():
        predict_results = {}
        for transition_type, transition_train_data in group_value["train_set"].items():
            ...
            model = tf.keras.models.Sequential(name="gru_model")  # optional name
            model.add(
                tf.keras.layers.GRU(1000, return_sequences=True, dropout=0.5, input_shape=(time_steps, train_set_x.shape[2])))
            model.add(tf.keras.layers.GRU(1000, return_sequences=False, dropout=0.5))
            model.add(tf.keras.layers.Dense(600, kernel_regularizer=regularization, kernel_initializer=initializer))
            model.add(tf.keras.layers.BatchNormalization())
            model.add(tf.keras.layers.ReLU())
...
def classifyBiGtuLastOneModel(shuffled_groups, time_steps):
    results = []
    for group_number, group_value in shuffled_groups.items():
        # input data
        train_set_x = group_value['train_feature_x'][:, :, 0:1040]
...
def classifyGtuSequenceModel(shuffled_groups, time_steps):
    results = []
    for group_number, group_value in shuffled_groups.items():
        # input data
        train_set_x = group_value['train_feature_x'][:, :, 0:1040]
        ...
        # model structure
        model = tf.keras.models.Sequential(name="gru_model")  # optional name
        model.add(tf.keras.layers.GRU(1000, return_sequences=True, dropout=0, input_shape=(time_steps, train_set_x.shape[2])))
