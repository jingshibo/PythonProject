...
def classifyConvLstmLastOneModel(shuffled_groups):
    ...
    for group_number, group_value in shuffled_groups.items():
        ...
        x = tf.keras.layers.Dense(1500, kernel_regularizer=regularization, kernel_initializer=initializer)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.ReLU()(x)
        x = tf.keras.layers.Dropout(0.5)(x)
        x = tf.keras.layers.Dense(700, kernel_regularizer=regularization, kernel_initializer=initializer)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.ReLU()(x)
        x = tf.keras.layers.Dropout(0.5)(x)
