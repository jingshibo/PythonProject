...
def combineNormalizedDataset(cross_validation_groups, window_per_repetition):
    ...
    for group_number, group_value in cross_validation_groups.items():
        ...
        train_norm_x = np.transpose((train_feature_x - np.mean(train_feature_x, axis=-1)[:, :, :, np.newaxis]) / np.std(train_feature_x, axis=-1)[:, :,
        :, np.newaxis],  (3, 0, 1, 2))
        test_norm_x = np.transpose((test_feature_x - np.mean(train_feature_x, axis=-1)[:, :, :, np.newaxis]) / np.std(train_feature_x, axis=-1)[:, :, :,
        np.newaxis],  (3, 0, 1, 2))
        ...
        train_norm_x_reshaped = np.transpose(np.reshape(train_norm_x, (window_per_repetition, -1, train_norm_x.shape[1],
            train_norm_x.shape[2], train_norm_x.shape[3]), order='F'), (1, 0, 2, 3, 4))
        test_norm_x_reshaped = np.transpose(np.reshape(test_norm_x, (window_per_repetition, -1, test_norm_x.shape[1],
            train_norm_x.shape[2], train_norm_x.shape[3]), order='F'), (1, 0, 2, 3, 4))
        ...
        train_onehot_y_reshaped = np.transpose(np.reshape(train_onehot_y, (window_per_repetition, -1, train_onehot_y.shape[1]), order='F'),
            (1, 0, 2))
        test_onehot_y_reshaped = np.transpose(np.reshape(test_onehot_y, (window_per_repetition, -1, test_onehot_y.shape[1]), order='F'),
            (1, 0, 2))

        # put training data and test data into one group
        normalized_groups[group_number] = {"train_feature_x": train_norm_x_reshaped, "train_int_y": train_int_y_reshaped,
            "train_onehot_y": train_onehot_y_reshaped, "test_feature_x": test_norm_x_reshaped, "test_int_y": test_int_y_reshaped,
            "test_onehot_y": test_onehot_y_reshaped}
