...
def createSlidingDataset(cross_validation_groups, initial_start=0, initial_end=16, shift=4):
    # initial_start is the start window position, initial_end is the end window position, shift is the number of window sliding (16ms, 32 samples)
    ...
    for group_number, group_value in emg_feature_all.items():
        for set_type, set_value in group_value.items():
            ...
            if set_type == 'train_set':  # combine the features from different window positions together
                for i in range(0, shift_range, shift):  # 0, 4, 8, 12, 16, 20, 24, 28, 32
                    emg_feature_value = selectSamples(set_value, start_index=initial_start + i, end_index=initial_end + i)  # 0, 16, 32, 48
                ...
                for i in range(0, shift_range, shift):  # 0, 4, 8, 12, 16, 20, 24, 28, 32
                    emg_feature_value = selectSamples(set_value, start_index=initial_start + i, end_index=initial_end + i)
                    emg_sliding_features[f'shift_{i}'] = emg_feature_value  # the keyword is the number of window shift
