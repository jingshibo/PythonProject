...
def build_cv_dataset_with_mix_data(new_emg_data, old_emg_data, gan_model, modes_generation, training_parameters, n_splits=5,
    ...
    # Step 2: Flatten original data
    all_keys = sorted(new_emg_data.keys())
    label_map = {key: i for i, key in enumerate(all_keys)}
    X_all = [sample[np.newaxis, :, :] for key in all_keys for sample in new_emg_data[key]]
    y_all = [label_map[key] for key in all_keys for _ in new_emg_data[key]]
    X_all = np.array(X_all).astype(np.float32)
    y_all = np.array(y_all).astype(np.int64)
    old_emg_dict = {key: [arr[np.newaxis, :, :] if arr.ndim == 2 else arr for arr in list_of_arrays] for key, list_of_arrays in
        old_emg_data.items()}
    ...
    for train_idx, test_idx in kf.split(X_all, y_all):
        ...
        for label, samples in train_data_by_label.items():
            ...
            if key in list(modes_generation.keys()):
                ...
                # N old samples
                gen_samples = old_emg_dict[key]
                gen_indices = np.random.choice(len(gen_samples), size=min(n_old_transition, len(gen_samples)), replace=False)
                replaced_train_dict[key].extend([gen_samples[i] for i in gen_indices])
